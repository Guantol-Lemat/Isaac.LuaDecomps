...
local function GetPlanetariumChance(myContext, level)
    ...
    if game.m_planetariumsVisited ~= 0 then
    ...
    local roomFilter = GameUtils.GetChallengeParams(myContext, game).m_roomFilter
    local treasureRoomsAllowed = not roomFilter[RoomType.ROOM_TREASURE]

    local treasureRoomsMissed = (stage - 1) - game.m_treasureRoomsVisited
